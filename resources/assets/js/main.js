...
(function(window) {
  ...
  async function loadDailyNotes(date) {
    ...
    try {
      ...
      // biarkan state.notes apa adanya
    }
      ...
      onClickDate(self, event) {
        ...
        if (btn) {
          ...
          if (dateDiv && dateDiv.dataset.vcDate) {
            loadDailyNotes(dateDiv.dataset.vcDate);
          }
        }
      },
      classes: {
  ...
  function setupGlobalEvents() {
    document.body.addEventListener('click', async (e) => {
      ...
      if (addExistingTag) {
        e.preventDefault(); addTag(addExistingTag.dataset.tagName);
      }
      ...
      if (e.target.classList.contains('remove-checklist-item')) {
        e.preventDefault(); removeChecklistItem(e.target);
      }
      ...
      if (toggleCheck) {
        ...
        try {
          ...
          if (index >= 0 && index < items.length) {
            ...
            state.setState('notes', updatedNotes);
            const icon = row.querySelector('.checklist-toggle'); const span = row.querySelector('span');
            if (newDone) {
              icon.classList.remove('bi-square'); icon.classList.add('bi-check-square-fill', 'text-success'); span.classList.add('text-decoration-line-through', 'text-muted');
            } else {
              icon.classList.remove('bi-check-square-fill', 'text-success'); icon.classList.add('bi-square'); span.classList.remove('text-decoration-line-through', 'text-muted');
            }
      ...
      if (forceDeleteBtn) {
        e.preventDefault(); if (confirm('Hapus permanen?')) {
          try {
      ...
      if (e.target.id === 'logout-btn') {
        localStorage.removeItem('telegram_token'); tgApp.showToast('Logout berhasil', 'success'); if (window.Telegram?.WebApp) window.Telegram.WebApp.close();
      }

      if (state.aiEnabled) {
        if (e.target.id === 'ai-search-btn' || e.target.closest('#ai-search-btn')) {
          e.preventDefault(); const query = document.getElementById('search-notes')?.value.trim();
          if (!query || query.length < 3) {
            tgApp.showToast('Ketik minimal 3 karakter', 'warning'); return;
          }
          tgApp.showLoading('AI sedang mencari...');
          try {
            const result = await api.aiSearch(query); const notes = extractData(result); state.setState('notes', notes); const lc = document.getElementById('notes-list-container'); if (lc) lc.innerHTML = Page.renderNotesGrid(notes); renderNotesPagination(); tgApp.showToast(`Ditemukan ${notes.length} catatan relevan`, 'success');
          }
          catch (err) {
        ...
        if (e.target.id === 'summarize-btn' || e.target.closest('#summarize-btn')) {
          e.preventDefault(); const note = state.currentNote; if (!note) return; const btn = document.getElementById('summarize-btn'); if (!btn) return;
          btn.disabled = true; const orig = btn.innerHTML; btn.innerHTML = '<span class="spinner-border spinner-border-sm me-1"></span>Meringkas...';
          try {
            const result = await api.summarizeNote(note.id); const sd = document.getElementById('ai-summary'); if (sd && result.summary) sd.innerHTML = `<div class="card bg-warning bg-opacity-10 border-warning p-3 mt-2"><h6 class="text-warning"><i class="bi bi-stars me-1"></i>Ringkasan AI</h6><p class="mb-0">${result.summary}</p></div>`; btn.innerHTML = '<i class="bi bi-check-lg me-1"></i> Teringkas'; setTimeout(() => {
    ...
    document.body.addEventListener('submit',
      async (e) => {
        ...
        if (form.id === 'quick-capture') {
          const title = form.querySelector('input[name="title"]')?.value.trim(); if (!title) return;
          try {
            const nn = await api.createNote({
              title
            }); state.setState('notes', [extractData(nn), ...state.notes]); form.reset(); tgApp.showToast('Catatan tersimpan', 'success'); navigateTo('#/notes/home');
          }
          catch (err) {
            tgApp.showToast(err.message, 'danger');
          }
    ...
    const debouncedSearch = helpers.debounce(async (keyword) => {
      ...
      try {
        const d = await api.getNotes({
          page: 1, search: keyword
        }); state.setState('notes', extractData(d)); state.setState('pagination', {
          currentPage: d.current_page || 1, lastPage: d.last_page || 1
        }); const lc = document.getElementById('notes-list-container'); if (lc) lc.innerHTML = Page.renderNotesGrid(state.notes); renderNotesPagination();
      }
      catch(err) {
        tgApp.showToast(err.message, 'danger');
      }
  ...
  async function init() {
    ...
    try {
      ...
      const tgUser = window.Telegram?.WebApp?.initDataUnsafe?.user; if (tgUser) state.setState('user', {
