...
(function(window) {
  ...
  async function loadDailyNotes(date) {
    ...
    const headingEl = document.getElementById('daily-date-heading');
    if (headingEl) {
      headingEl.textContent = selectedDate.toLocaleDateString('id-ID', {
        weekday: 'long', day: 'numeric', month: 'long', year: 'numeric'
      });
    }
    const notesForSelected = state.notes.filter(n => n.note_date === date);
    const listContainer = document.getElementById('daily-notes-list');
    if (listContainer) {
      listContainer.innerHTML = notesForSelected.length
      ? notesForSelected.map(n => `<div class="card note-card text-white mb-2"><div class="card-body"><h6>${helpers.escapeHtml(n.title)}</h6>${n.content ? `<p class="small text-muted">${helpers.stripHtml(n.content).substring(0, 100)}...</p>`: ''}<a href="#/notes/${n.id}" class="btn btn-sm btn-outline-light">Buka</a></div></div>`).join(''): '<p class="text-muted">Belum ada catatan untuk hari ini.</p>';
    }

    const remindersForDate = state.reminders.filter(r => r.remind_at && r.remind_at.slice(0, 10) === date);
    const reminderContainer = document.getElementById('daily-reminders-list');
    if (reminderContainer) {
      reminderContainer.innerHTML = remindersForDate.length
      ? `<h6 class="text-warning mt-3">⏰ Pengingat</h6>` + remindersForDate.map(r => {
        ...
        let badge = '';
        if (isCompleted) badge = ' <span class="badge bg-success">Selesai</span>';
        else if (isNotified) badge = ' <span class="badge bg-info">Terkirim</span>';
        return `<div class="card bg-warning bg-opacity-10 border-warning text-white mb-2">
        <div class="card-body">
        <h6>${title}${badge}</h6>
        <small>${dateStr}</small>
        ${!isCompleted ? `<button data-complete-reminder="${r.id}" class="btn btn-sm btn-outline-success mt-1"><i class="bi bi-check"></i> Selesaikan</button>`: ''}
        </div>
        </div>`;
      }).join(''): '';
    }
  ...
  async function updateCalendarPopups() {
    if (!calendar) return;
    let datesWithNotes = [];
    let datesWithReminders = [];
    try {
      datesWithNotes = await api.getDatesWithNotes();
    } catch (err) {}
    try {
      datesWithReminders = await api.getDatesWithReminders();
    } catch (err) {}
    const popups = {};
    ...
    calendar = new Calendar(container, {
      ...
      onClickDate(self, event) {
        ...
        if (btn) {
          ...
          if (dateDiv && dateDiv.dataset.vcDate) loadDailyNotes(dateDiv.dataset.vcDate);
        }
      },
      classes: {
  ...
  async function renderRoute(p) {
    ...
    if (p.full === '/notes/home') {
      ...
      if (state.reminders.length === 0) {
        const d = await api.getReminders(); state.setState('reminders', extractData(d));
      }
      html = Page.home();
      ...
      if (!state.user?.id) {
        try {
          const profile = await api.getProfile(); state.setState('user', profile);
        } catch (err) {
      ...
      const d = await api.getTrashedNotes(); state.setState('trashedNotes', d.data || d);
      html = Page.trash();
    } else if (p.isEdit && p.parts.length >= 2) {
      const id = p.parts[1]; const d = await api.getNote(id);
  ...
  function updateActiveNav(path) {
    document.querySelectorAll('.nav-link').forEach(link => {
      ...
      link.classList.toggle('active-link', path.startsWith(route));
      link.classList.toggle('text-warning', path.startsWith(route));
    });
  }
  ...
  function setupGlobalEvents() {
    document.body.addEventListener('click', async (e) => {
        ...
        if (confirm('Hapus catatan ini?')) {
          try {
            ...
            if (state.activeRoute === '/notes/daily') {
              ...
              initDailyCalendar();
            } else window.location.hash = '#/notes/all';
          } catch (err) {
            tgApp.showToast(err.message, 'danger');
          }
        }
      ...
      if (completeBtn) {
        ...
        try {
          ...
          if (state.activeRoute === '/notes/reminders') document.getElementById('app-content').innerHTML = Page.reminders();
          else if (state.activeRoute === '/notes/home') document.getElementById('app-content').innerHTML = Page.home();
        } catch (err) {
          tgApp.showToast(err.message, 'danger');
        }
        return;
      ...
      if (e.target.id === 'quick-reminder-save') {
        ...
        if (!reminderAt) {
          tgApp.showToast('Pilih tanggal dan waktu', 'warning'); return;
        }
        ...
        try {
          ...
          state.setState('notes', state.notes.map(n => n.id === note.id ? {
            ...n, reminder: {
      ...
      if (e.target.id === 'add-checklist-btn') {
        ...
        if (input) {
          const text = input.value.trim(); if (text) {
      ...
      if (e.target.classList.contains('remove-checklist-item')) {
        removeChecklistItem(e.target); return;
      }
      ...
      if (toggleCheck) {
        ...
        try {
          ...
          if (index >= 0 && index < items.length) {
            const item = items[index]; const text = item.text || item; const newDone = !item.done;
            items[index] = {
              text,
              done: newDone
            };
            ...
            if (newDone) {
              icon.classList.replace('bi-square', 'bi-check-square-fill'); icon.classList.add('text-success'); span.classList.add('text-decoration-line-through', 'text-muted');
            } else {
              icon.classList.replace('bi-check-square-fill', 'bi-square'); icon.classList.remove('text-success'); span.classList.remove('text-decoration-line-through', 'text-muted');
            }
      ...
      if (restoreBtn) {
        try {
          await api.restoreNote(restoreBtn.dataset.restoreNote); tgApp.showToast('Catatan dipulihkan', 'success'); navigateTo(window.location.hash);
        }
        catch (err) {
          tgApp.showToast(err.message, 'danger');
        }
      ...
      if (forceDeleteBtn && confirm('Hapus permanen?')) {
        try {
          await api.forceDeleteNote(forceDeleteBtn.dataset.forceDeleteNote); tgApp.showToast('Catatan dihapus permanen', 'success'); navigateTo(window.location.hash);
        }
        catch (err) {
          tgApp.showToast(err.message, 'danger');
        }
      ...
      if (state.aiEnabled) {
        if (e.target.id === 'ai-search-btn' || e.target.closest('#ai-search-btn')) {
          ...
          if (!query || query.length < 3) {
            tgApp.showToast('Ketik minimal 3 karakter', 'warning'); return;
          }
          ...
          try {
            ...
            const lc = document.getElementById('notes-list-container');
            if (lc) lc.innerHTML = Page.renderNotesGrid(notes);
            renderNotesPagination();
        ...
        if (e.target.id === 'summarize-btn' || e.target.closest('#summarize-btn')) {
          ...
          try {
            ...
            const sd = document.getElementById('ai-summary');
            if (sd && result.summary) sd.innerHTML = `<div class="card bg-warning bg-opacity-10 border-warning p-3 mt-2"><h6 class="text-warning"><i class="bi bi-stars me-1"></i>Ringkasan AI</h6><p class="mb-0">${result.summary}</p></div>`;
            btn.innerHTML = '<i class="bi bi-check-lg me-1"></i> Teringkas';
            setTimeout(() => {
              btn.disabled = false; btn.innerHTML = orig;
            }, 3000);
          } catch (err) {
            tgApp.showToast('Gagal merangkum: ' + err.message, 'danger'); btn.disabled = false; btn.innerHTML = orig;
          }
          return;
        }
      }
    ...
    document.body.addEventListener('submit',
      async (e) => {
        ...
        if (form.id === 'quick-capture') {
          ...
          if (!title) return;
          try {
            const nn = await api.createNote({
              title
            });
            state.setState('notes', [extractData(nn), ...state.notes]);
            form.reset();
        ...
        } else if (form.id === 'daily-quick-capture') {
          const title = form.querySelector('input[name="title"]')?.value.trim();
          if (!title) return;
          ...
          try {
            ...
            if (noteDate) window.location.hash = `#/notes/daily?date=${noteDate}`;
            else window.location.hash = '#/notes/all';
          } catch (err) {
    ...
    const debouncedSearch = helpers.debounce(async (keyword) => {
      ...
      try {
        ...
        const lc = document.getElementById('notes-list-container');
        if (lc) lc.innerHTML = Page.renderNotesGrid(state.notes);
        renderNotesPagination();
      } catch (err) {
        tgApp.showToast(err.message, 'danger');
      }
  ...
  async function init() {
    state.setState('activeDate',
      helpers.getToday());
    try {
      const profile = await api.getProfile(); state.setState('user',
        profile);
    } catch (err) {
      const tgUser = window.Telegram?.WebApp?.initDataUnsafe?.user;
      if (tgUser) state.setState('user', {
        id: tgUser.id, first_name: tgUser.first_name, last_name: tgUser.last_name, username: tgUser.username || null, photo_url: tgUser.photo_url || null
      });
    }
    try {
      const tagsData = await api.getTags(); state.setState('allTags', tagsData || []);
    } catch (err) {}
    window.addEventListener('hashchange', () => navigateTo(window.location.hash));
    if (!window.location.hash) window.location.hash = '#/notes/home'; else await navigateTo(window.location.hash);
    setupGlobalEvents();
  }
